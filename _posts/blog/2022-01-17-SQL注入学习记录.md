...
于是，为了减少自己的工程量，此处自己根据原理就编写了一份脚本解决（后续相关查找皆配合了相关脚本得以解决）。
...
由上图得知flag表的第一个字段的第一个值ASCII码值为102，即f，查完后该字段值为flag；另一个表名news的第一个字段名为id，第二个字段名为data。此处为查找方便故自己编写了一份脚本解决：
...
上图可知，news表中id字段第一行第一个值的ASCII码值为49，即1。由于此处news表不是我们的目标，遂直接跑flag表，直接上脚本跑得flag表，结果如下：
